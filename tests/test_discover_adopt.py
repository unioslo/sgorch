...
from sgorch.config import DeploymentConfig, ConnectivityConfig, RouterConfig, SlurmConfig, SGLangConfig, HealthConfig, PolicyConfig
...
def test_extract_from_logs_reads_READY_marker_and_ports(tmp_path):
    ...
    wd = WorkerDiscovery(cfg, slurm=None)  # slurm not used here
...
def test_compute_from_job_info_guesses_ports_with_tcp_probe(monkeypatch, tmp_path):
    cfg = _dep_cfg(tmp_path)
    wd = WorkerDiscovery(cfg, slurm=None)
